  ...
  | .const name _ args =>
    ...
    | some (.ctorInfo ctorVal) =>
      ...
      if type.isStruct then
        -- For struct types, only keep non-erased fields.
        let structArgs? : Option (Array Arg) ← do
          let mut result : Array Arg := #[]
          let mut valid := true
          for h : i in *...fields.size do
            match fields[i], irArgs[i]! with
            | .erased, _ =>
              pure ()
            | _, .var varId =>
            ...
            | _, .erased =>
              valid := false
          pure (if valid then some result else none)
        match structArgs? with
        | some structArgs =>
          return .vdecl objVar type (.ctor ctorInfo structArgs) (← lowerCode k)
        | none =>
          -- Can happen after transforms over unreachable code.
          return .unreachable
      else
